...
public class LazyMastiffRow extends LazyMastiffRowBase {
  ...
  private HashSet<Integer> validColumnsSet; // set version of selected columns
  private HashMap<Integer, Integer> validCfToIdx; // cf# to its idx in validCfs

  Object[] fields; // store java primitives deserialized from RowWritable
  ...
  public void init(RowWritable rw, MTableDesc tblDesc) {
    ...
    if ((this.tblDesc == null) ||
        (this.tblDesc != tblDesc && !this.tblDesc.equals(tblDesc))) {
      this.tblDesc = tblDesc;
      cells = new HashMap<Integer, Cell>();
      validCfToIdx = new HashMap<Integer, Integer>();
      if (result.getIsFirst() == 1) {
        validCfs = rw.getValidCfs();
        List<Integer> validColumns = rw.getValidColumns();
        validColumnsSet = new HashSet<Integer>(validColumns);
      }
      int i = 0;
      for (Integer cfIdx : validCfs) {
        if (!cells.containsKey(cfIdx)) {
          Cell curCell = new Cell();
          curCell.init(Arrays.asList(tblDesc.clusterTypes[cfIdx]));
          cells.put(cfIdx, curCell);
        }
        validCfToIdx.put(cfIdx, i);
        i++;
      }
    }
    setParsed(false);
  ...
  private Object uncheckedGetField(int fieldID) {
    ...
    if (!fieldsInited[fieldID]) {
      ...
      if (validColumnsSet.contains(fieldID)) {
        int idx = validCfToIdx.get(cdesc.cf);
        ValPair curvp = result.getVps().get(idx);
   ...
   * @param vp
   * @param loc
   *          Offset and length of current field
  ...
  private Object getJavaPrimitiveObject(int fieldID, ValPair vp, ClmLocation loc) {
    ...
    //  int length = loc.length;
      ...
      case DATE:
        long dt = Bytes.toLong(vp.data, offset);
