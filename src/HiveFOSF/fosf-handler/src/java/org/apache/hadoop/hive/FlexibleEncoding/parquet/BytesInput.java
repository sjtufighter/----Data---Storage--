...
abstract public class BytesInput {
	...
	public static BytesInput from(byte[] in) {
		if (DEBUG) {
      LOG.debug("BytesInput from array of " + in.length + " bytes");
    }
	...
	public static BytesInput from(byte[] in, int offset, int length) {
		if (DEBUG) {
      LOG.debug("BytesInput from array of " + length + " bytes");
    }
	...
	public byte[] toByteArray() throws IOException {
		...
		// out.close();
		if (DEBUG) {
      LOG.debug("converted " + size() + " to byteArray of " + baos.size() + " bytes");
    }
		return baos.getBuf();
	}
  public  byte[]    getBufferSize() throws IOException{
	...
	private static class StreamBytesInput extends BytesInput {
		...
		public void writeAllTo(OutputStream out) throws IOException {
			if (DEBUG) {
        LOG.debug("write All "+ byteCount + " bytes");
      }
			// TODO: more efficient
    ...
    public byte[] toByteArray() throws IOException {
			if (DEBUG) {
        LOG.debug("read all "+ byteCount + " bytes");
      }
	...
	private static class SequenceBytesIn extends BytesInput {
		...
		public void writeAllTo(OutputStream out) throws IOException {
			for (BytesInput input : inputs) {
				if (DEBUG) {
          LOG.debug("write " + input.size() + " bytes to out");
        }
				if (DEBUG && input instanceof SequenceBytesIn) {
          LOG.debug("{");
        }
