  ...
  protected void checkAndFallbackIfNeeded() {
    if (dictionaryByteSize > maxDictionaryByteSize || getDictionarySize() > MAX_DICTIONARY_ENTRIES) {
      // if the dictionary reaches the max byte size or the values can not be encoded on 4 bytes anymore.
    	...
    	System.out.println("fallBackToPlainEncoding and dictionaryByteSize=  "+dictionaryByteSize+" getDictionarySize() = " + getDictionarySize());
    	fallBackToPlainEncoding();
    }
  }

  private void fallBackToPlainEncoding() {
	  System.out.println("fallBackToPlainEncoding");
	  System.out.println("fallBackToPlainEncoding");
	  System.out.println("fallBackToPlainEncoding");
	  if (Log.DEBUG)
    ...
    if (!dictionaryTooBig && getDictionarySize() > 0) {
      ...
      try {
        ...
        if (firstPage && ((bytes.size() + dictionaryByteSize) > rawDataByteSize)) {
          System.out.println("bytes.size()  "+bytes.size()+"dictionaryByteSize" +dictionaryByteSize+"rawDataByteSize"+rawDataByteSize);
        	fallBackToPlainEncoding();
        } else {
...
//a page data  wm
    ...
    //added  by wm
    public void WriteDictionaryToDisk(String[] s) throws IOException{
  System.out.println("write  just zidian to  disk");
        // return a dictionary only if we actually used it
        PlainValuesWriter dictionaryEncoder = new PlainValuesWriter(lastUsedDictionaryByteSize);
        it.unimi.dsi.fastutil.ints.IntIterator intIterator = intDictionaryContent.keySet().iterator();
        // write only the part of the dict that we used
      ...
      System.out.println("lastUsedDictionarySize  "+lastUsedDictionarySize);
    	...
    	dos.writeInt(lastUsedDictionarySize);
    	 System.out.println("dictionaryEncoder.getBytes().toByteArray()"+dictionaryEncoder.getBytes().toByteArray().length);
    ...
    //	dictionaryEncoder.getBytes().writeAllTo(dos);
    	...
    	dos.close();
    	System.out.println("finish  write  just zidian to  disk");
